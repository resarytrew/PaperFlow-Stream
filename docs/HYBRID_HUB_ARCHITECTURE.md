# Чистовик Architecture

## 1. Цель

Чистовик должен выглядеть для учителя как обычный веб-сервис, но персональные
...
Чистовик рассматривает как допустимые Hub-адреса только:
...
`chistovik.school.local`, а firewall допускает только школьную сеть.
...
8. Оба токена привязаны к workspace и browser client; API-token дополнительно
   проверяется по точному Origin.
...
Кросс-сайтовые браузерные запросы без `Origin` блокируются, кроме узкого media-
маршрута. Загрузка изображения через `<img>` разрешается только при наличии
валидного read-only media-token и правильного workspace. Анонимная ссылка или
API-token вместо media-token получают отказ.
